...
export class CustomerService {
    ...
    getCustomerProfile(): Observable<CustomerProfile[]> {
        const fullUrl = this.getBaseUrl() + this.custUrl;
        const resData = this._http.get<CustomerProfile[]>(fullUrl, { headers: this._header })
                        .pipe(
    ...
    save(saveCustomerProfile: CustomerProfile): Observable<any> {
        const fullUrl = this.getBaseUrl() + this.custUrl;
        const resData = this._http.post(fullUrl, saveCustomerProfile, { headers: this._header })
                        .pipe(
                            catchError(this.handleError),
                        );
        return resData;
    ...
    edit(saveShipper: CustomerProfile): Observable<any> {
        const fullUrl = this.getBaseUrl() + this.custUrl;
        const resData = this._http.put(fullUrl, saveShipper, { headers: this._header })
                        .pipe(
                            catchError(this.handleError),
                        );
        return resData;
    ...
    delete(delId: number): Observable<any> {
        const fullUrl = this.getBaseUrl() + this.custUrl + '/' + delId;
        const resData = this._http.delete(fullUrl)
                        .pipe(
                            catchError(this.handleError),
                        );
        return resData;
